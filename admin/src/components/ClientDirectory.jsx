...
function ClientModal({ client, onClose, onSave }) {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) {
      ...
      newErrors.name = "Client name can only contain letters and spaces.";
    }
    ...
    if (!phone) {
      ...
      if (!cleanPhone.startsWith('+94') && !cleanPhone.startsWith('0')) {
        newErrors.phone = "✅ Phone number must start with +94 or 0.";
      } else {
        // Check digit count
        let expectedDigits;
        if (cleanPhone.startsWith('+94')) {
          expectedDigits = 12; // +94 + 9 digits
          if (cleanPhone.length !== 12) {
            newErrors.phone = "🔢 Phone number starting with +94 must have exactly 12 digits total.";
          }
        } else if (cleanPhone.startsWith('0')) {
          expectedDigits = 10; // 0 + 9 digits
          if (cleanPhone.length !== 10) {
            newErrors.phone = "🔢 Phone number starting with 0 must have exactly 10 digits total.";
          }
        }
        
        // Check for repeated digits (7 or more times)
        const repeatedDigits = /(\d)\1{6,}/;
        if (repeatedDigits.test(cleanPhone)) {
          newErrors.phone = "❌ Phone number cannot have the same digit repeating 7 or more times.";
        }
        
        // Sri Lankan mobile number validation (only if no other errors)
        if (!newErrors.phone) {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'phone' && processedValue.trim()) {
      ...
      if (!cleanPhone.startsWith('+94') && !cleanPhone.startsWith('0')) {
        setErrors({ ...errors, phone: "✅ Phone number must start with +94 or 0." });
      } else if (cleanPhone.startsWith('+94') && cleanPhone.length > 0 && cleanPhone.length !== 12) {
        setErrors({ ...errors, phone: "🔢 Phone number starting with +94 must have exactly 12 digits total." });
      } else if (cleanPhone.startsWith('0') && cleanPhone.length > 0 && cleanPhone.length !== 10) {
        setErrors({ ...errors, phone: "🔢 Phone number starting with 0 must have exactly 10 digits total." });
      } else {
        // Check for repeated digits (7 or more times)
        const repeatedDigits = /(\d)\1{6,}/;
        if (repeatedDigits.test(cleanPhone)) {
          setErrors({ ...errors, phone: "❌ Phone number cannot have the same digit repeating 7 or more times." });
        } else {

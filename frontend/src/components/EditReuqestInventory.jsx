...
import { updateInventoryRequest } from "../apis/inventoryRequestApi";

const EditRequestInventory = ({ request, onClose, onUpdated }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data } = await updateInventoryRequest(request._id, payload);
      onUpdated(data);
      window.location.reload();
    } catch (e) {
      setSubmitError(
        e.response?.data?.message || "Failed to update inventory request"
      );
    } finally {
      setLoading(false);
    }

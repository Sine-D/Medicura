...
// Client-only demo cart using localStorage to avoid backend dependencies
...
const MyCart = () => {
  ...
  useEffect(() => {
    ...
    setCart({ ...stored, total: calculateCartTotal(stored) });
    setError(null);
    setLoading(false);
  }, []);
  ...
  const handleUpdateQuantity = (itemId, newQuantity) => {
    ...
    try {
      ...
      updated.items = updated.items.map((it) =>
        it.inventoryItem._id === itemId ? { ...it, quantity: newQuantity } : it
      );
      updated.total = calculateCartTotal(updated);
      setCart(updated);
  ...
  const handleRemoveItem = (itemId) => {
    ...
    try {
      ...
      updated.items = updated.items.filter(
        (it) => it.inventoryItem._id !== itemId
      );
      updated.total = calculateCartTotal(updated);
      setCart(updated);
      saveCartToStorage(updated);
  ...
  const handleClearCart = () => {
    ...
    const cleared = { items: [], total: 0 };
    setCart(cleared);
    saveCartToStorage(cleared);
  };
  ...
  // Always render cart in demo mode
